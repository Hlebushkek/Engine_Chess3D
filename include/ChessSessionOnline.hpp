...
{
public:
    ChessSessionOnline(std::shared_ptr<ChessClient> client, std::shared_ptr<ChessBoard> board, const Lobby& lobby, const User& user);
    ~ChessSessionOnline();
...
private:
    std::shared_ptr<ChessClient> m_client;
